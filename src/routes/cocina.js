...
router.get('/pendientes', verificarToken, verificarRol([1, 4]), async (req, res) => {
  try {
    const query = `
      SELECT 
        p.id as pedido_id, 
...
router.patch('/pedidos/:id/estado', verificarToken, verificarRol([1, 4]), async (req, res) => {
  ...
  try {
    ...
    if (nuevo_estado === 'listo') {
      ...
      const mesaQuery = await pool.query(`
        SELECT m.numero FROM cuentas c
        JOIN mesas m ON c.mesa_id = m.id
        WHERE c.id = $1
      `, [pedidoActualizado.cuenta_id]);

      const mesaNumero = mesaQuery.rows[0].numero;

      io.emit('pedido_listo_para_entregar', {
        mensaje: `¡Platillo listo para la Mesa ${mesaNumero}!`,
        mesa: mesaNumero,
