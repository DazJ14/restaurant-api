...
router.get('/cuenta/:cuenta_id', verificarToken, verificarRol([1, 2, 3]), async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Cuenta no encontrada o sin pedidos' });
    }
...
router.post('/pagar', verificarToken, verificarRol([1, 2, 3]), async (req, res) => {
  try {
    ...
    for (const pago of pagos) {
      await pool.query(
        `INSERT INTO pagos (cuenta_id, monto, metodo_pago, cliente_nombre) VALUES ($1, $2, $3, $4)`,
    ...
    io.emit('mesas_actualizadas', {
      mensaje: `La Mesa ${mesaPrincipalId} ha sido pagada y liberada`,
      accion: 'liberacion'
    });
